...
if [ "$choice" == "0" ]; then
    ...
    for var_name in "${!options[@]}"; do
        options["$var_name"]=$(select_option "$var_name")
    done
fi
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .flex-container {
            display: ${options["flex-container-display"]};
            flex-flow: ${options["flex-container-flex-flow"]};
            justify-content: ${options["flex-container-justify-content"]};
            align-items: ${options["flex-container-align-items"]};
